...
for run in range(num_runs):
    skf = StratifiedKFold(n_splits=num_folds, shuffle=True, random_state=run)
    for fold, (train_indexes, val_indexes) in enumerate(skf.split(X_train, Y_train)):
        ...
        result_val_fold = pd.DataFrame({'y_test': y_val.flatten(), 'y_pred_test': y_pred_val.flatten()})
        result_val_fold.to_csv(f'val_results_run{run}_fold{fold}.csv', index=False)
